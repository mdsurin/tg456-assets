...
(function () {
  'use strict';
  const sports = [['soccer', 'ฟุตบอล'], ['basketball', 'บาสเกตบอล'], ['volleyball', 'วอลเลย์บอล'], ['baseball', 'เบสบอล'], ['mma', 'MMA / UFC']];
  ...
  function team(data, demo) {
    ...
    if (!demo && /^https:\/\/media\.api-sports\.io\/(?:football|basketball|volley|baseball|mma)\/(?:teams|fighters)\/\d+\.png$/.test(data.logo || '')) {
  ...
  function card(item, demo) {
    ...
    state.append(node('strong', 'tg-sports-match-value', item.resultText || (item.isLive || finished ? score : date ? formatTime.format(date) : '–')));
    ...
    async function load() {
      ...
      try {
        ...
        if (demo) payload = config.demoData[active] || [];
          ...
          if (!feed.sports[active]) { empty('ยังไม่ได้เชื่อมข้อมูลจริงสำหรับกีฬานี้'); status.textContent = 'กำลังเตรียมข้อมูลสำหรับกีฬานี้'; return; }
          payload = {...feed.sports[active], updatedAt:feed.sports[active].updatedAt || feed.updatedAt};
          if (payload.error && !payload.data?.length) throw new Error('Sport unavailable');
        }
...
(function(){
 ...
 function install(){
  ...
  const root=el('section','lwd-widget');root.id='tg456-withdrawals';root.setAttribute('aria-label','ยอดถอนล่าสุด — ข้อมูลตัวอย่าง');
  const style=el('style','');style.textContent=`#tg456-withdrawals{padding:0;margin:16px auto;border:0;border-radius:0;background:transparent;box-shadow:none;max-width:1440px;width:100%;min-width:0;color:#f8f5ff;font-family:Tahoma,Arial,sans-serif;overflow:hidden}#tg456-withdrawals *{box-sizing:border-box}#tg456-withdrawals .tw-head{display:flex;align-items:center;gap:10px;background:linear-gradient(100deg,#762bc0,#34144e 65%,transparent);border-radius:22px;padding:9px 14px;margin-bottom:12px}#tg456-withdrawals h2{font-size:17px;margin:0;flex:1;color:#fff}#tg456-withdrawals .tw-demo{font-size:11px;color:#e5cafa;white-space:nowrap}#tg456-withdrawals .tw-toggle{border:1px solid #9b67c2;background:#29113f;color:#eee0ff;border-radius:16px;padding:4px 10px;font:inherit;font-size:11px;cursor:pointer}#tg456-withdrawals .tw-window{overflow-x:auto;scrollbar-width:none;border-radius:14px}#tg456-withdrawals .tw-window::-webkit-scrollbar{display:none}#tg456-withdrawals .tw-track{display:flex;width:max-content;animation:tg-withdraw-scroll 64s linear infinite}#tg456-withdrawals .tw-group{display:flex;gap:12px;padding-right:12px;flex:none}#tg456-withdrawals .tw-card{display:flex;align-items:flex-start;gap:10px;width:260px;padding:12px;border:1px solid #603083;border-radius:14px;background:linear-gradient(115deg,#140b20,#351449);min-height:96px}#tg456-withdrawals .tw-bank{width:40px;height:40px;object-fit:contain;border-radius:10px;flex:none}#tg456-withdrawals .tw-details{min-width:0}#tg456-withdrawals .tw-user{font-size:13px;font-weight:bold;white-space:nowrap}#tg456-withdrawals .tw-amount{font-size:21px;color:#39d5a0;font-weight:bold;line-height:1.4;white-space:nowrap}#tg456-withdrawals .tw-amount small{font-size:11px}#tg456-withdrawals .tw-note{font-size:10px;color:#bfaacf;margin-top:2px}#tg456-withdrawals:hover .tw-track,#tg456-withdrawals:focus-within .tw-track,#tg456-withdrawals[data-paused=true] .tw-track{animation-play-state:paused}@keyframes tg-withdraw-scroll{to{transform:translateX(-50%)}}@media(max-width:600px){#tg456-withdrawals{margin:12px auto}#tg456-withdrawals h2{font-size:14px}#tg456-withdrawals .tw-head{gap:6px;padding:8px 10px}#tg456-withdrawals .tw-card{width:235px;padding:10px;min-height:88px}#tg456-withdrawals .tw-amount{font-size:19px}#tg456-withdrawals .tw-demo{font-size:10px}}@media(prefers-reduced-motion:reduce){#tg456-withdrawals .tw-track{animation:none}#tg456-withdrawals .tw-group[aria-hidden=true]{display:none}}`;
  document.head.append(style);
  const head=el('div','tw-head'),toggle=el('button','tw-toggle','หยุด');toggle.type='button';toggle.setAttribute('aria-label','หยุดการเลื่อนรายการถอน');toggle.setAttribute('aria-pressed','false');
  toggle.addEventListener('click',()=>{const paused=root.dataset.paused!=='true';root.dataset.paused=String(paused);toggle.textContent=paused?'เล่นต่อ':'หยุด';toggle.setAttribute('aria-pressed',String(paused));toggle.setAttribute('aria-label',paused?'เล่นการเลื่อนรายการถอนต่อ':'หยุดการเลื่อนรายการถอน');});
  head.append(el('h2','','ยอดถอนล่าสุด'),el('span','tw-demo','ข้อมูลตัวอย่าง'),toggle);
  ...
  examples.forEach(([bank,name,id,amount])=>{const card=el('article','tw-card'),logo=el('img','tw-bank');logo.src='/g_assets/img/bank-logo/'+bank+'.svg';logo.alt=name;logo.width=40;logo.height=40;logo.addEventListener('error',()=>{logo.replaceWith(el('span','tw-bank','฿'));},{once:true});const detail=el('div','tw-details'),value=el('div','tw-amount',amount.toLocaleString('en-US',{minimumFractionDigits:2})+' ');value.append(el('small','','THB'));detail.append(el('div','tw-user','demo*****'+id),value,el('div','tw-note','ตัวอย่าง • ไม่ใช่รายการถอนจริง'));card.append(logo,detail);group.append(card);});
...
(function(){
 ...
 function install(){
  ...
  const style=el('style');style.textContent=`#tg456-lottery.tg-sports{color:#f8f5ff;background:#0a0416;border:1px solid #422160;border-radius:18px;padding:18px;font-family:Tahoma,Arial,sans-serif;box-sizing:border-box;max-width:1440px;margin:18px auto;width:auto}#tg456-lottery header{margin-bottom:12px;gap:10px}#tg456-lottery h2{font-size:20px}#tg456-lottery .tg-sports-subtitle{font-size:11px;margin-top:4px}#tg456-lottery .tg-sports-arrow{width:30px;height:30px;font-size:18px}#tg456-lottery .tg-sports-tabs{padding:4px;gap:3px;margin-bottom:14px}#tg456-lottery .tg-sports-tab{padding:9px 16px;font-size:13px}#tg456-lottery .tg-sports-track{gap:12px;padding-bottom:6px}#tg456-lottery .tg-sports-card{flex:0 0 min(310px,88%);border-radius:16px}#tg456-lottery .tg-sports-card-head{display:grid;grid-template-columns:1fr auto;gap:4px 8px;padding:10px 12px;font-size:12px}#tg456-lottery .tg-sports-league{grid-column:1;grid-row:1}#tg456-lottery time{grid-column:1;grid-row:2;font-size:10px;color:#d4c3e4}#tg456-lottery .tg-sports-badge{grid-column:2;grid-row:1 / 3;padding:4px 7px;font-size:11px}#tg456-lottery .tg-lotto-body{padding:10px;display:grid;gap:6px;min-height:100px}#tg456-lottery .tg-lotto-row{display:flex;align-items:center;justify-content:space-between;gap:8px;border-radius:8px;background:#27133f;padding:7px 9px;font-size:11px}#tg456-lottery .tg-lotto-row span{flex:0 0 82px}#tg456-lottery .tg-lotto-row strong{color:#edb8ff;font-size:17px;letter-spacing:.6px;text-align:right;overflow-wrap:anywhere;min-width:0;line-height:1.3}#tg456-lottery .tg-lotto-row:first-child strong{font-size:21px}#tg456-lottery .tg-lotto-wait{align-self:center;text-align:center;color:#ccbfdf;padding:16px 8px;font-size:13px}#tg456-lottery .tg-sports-status:empty{display:none}@media(max-width:600px){#tg456-lottery.tg-sports{padding:12px;border-radius:16px;margin:12px 0}#tg456-lottery h2{font-size:18px}#tg456-lottery .tg-sports-tab{padding:8px 12px;font-size:12px}#tg456-lottery .tg-sports-card{flex-basis:min(290px,90%)}}`;
  document.head.append(style);
  ...
  root.append(header,tabs,track,status);
